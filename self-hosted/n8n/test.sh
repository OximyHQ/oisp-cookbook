#!/bin/bash
# Test script for self-hosted/n8n
#
# This script:
# 1. Starts n8n with OISP sensor
# 2. Waits for n8n to be ready
# 3. Creates OpenAI credentials via API
# 4. Imports and executes a test workflow
...
if [ -z "$CRED_ID" ]; then
    echo "Warning: Could not create credentials (may already exist)"
    CRED_ID="1"
fi
